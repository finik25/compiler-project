...
**Текущий этап (Sprint 5):** Генерация x86-64 ассемблера, пролог/эпилог функций, работа со стеком, вызов runtime-функций.
...
8. **Генерация x86-64 ассемблера** (Sprint 5):
   ```bash
   # Собрать ассемблерный файл
   compiler compile --input examples/simple_add.src --output program.asm

   # Собрать и запустить исполняемый файл (требуются nasm и ld)
   compiler compile --input examples/simple_add.src --run

   # Отключить регистровую аллокацию (стековый режим)
   compiler compile --input examples/simple_add.src --no-regalloc
   ```
   Для опции `--run` необходимы установленные **NASM** и **GNU ld** (доступные в PATH).

9. **Запуск тестов**:
   ```bash
   python -m pytest tests/ -v
   ```
   Все тесты (лексика, препроцессор, синтаксис, семантика, IR, кодогенерация) должны проходить успешно.
...
│   ├── codegen/                   # Генерация x86-64 кода (Sprint 5)
│   │   ├── __init__.py
│   │   ├── x86_generator.py       # Основной генератор ассемблера
│   │   ├── stack_frame.py         # Управление стеком
│   │   ├── abi.py                 # Регистры и ABI
│   │   ├── register_allocator.py  # Линейная регистровая аллокация
│   │   ├── liveness.py            # Анализ интервалов жизни
│   │   └── runtime/               # runtime.asm (syscall-функции)
...
│   ├── ir/                        # Тесты IR (valid)
│   └── codegen/                   # Тесты кодогенерации (valid)
├── .gitignore
...
## Генерация x86-64 кода (Sprint 5)

Кодогенератор преобразует IR в ассемблер NASM для платформы x86-64, следуя System V AMD64 ABI. Все локальные переменные и временные значения размещаются на стеке (отрицательные смещения от `rbp`). Поддерживаются целочисленные операции, сравнения, условные и безусловные переходы, циклы, вызовы функций (до 6 целочисленных аргументов). Встроенная runtime-библиотека предоставляет функции `print_int`, `print_char`, `exit` и точку входа `_start`.

### Пример трансляции

Исходный код (`examples/simple_add.src`):
```c
fn main() -> int {
    int a = 2;
    int b = 3;
...
Сгенерированный ассемблер (упрощённо):
```asm
main:
    push rbp
    mov rbp, rsp
...
### Регистровая аллокация (stretch goal)

В модуле `codegen` реализован линейный сканирующий аллокатор (LSRA) для локального распределения регистров внутри базовых блоков. Однако по умолчанию аллокация отключена (используется стековая модель) для стабильности. Включить её можно снятием флага `--no-regalloc` (но на данный момент опция оставлена для обратной совместимости).
...
- (опционально) **NASM** и **GNU ld** для сборки и выполнения сгенерированного кода (опция `--run`)
